...
class VG(Dataset):
    ...
    def __init__(self, mode, data_dir, filter_empty_rels=True, num_im=-1, num_val_im=5000,
        ...
        if VG.split == 'stanford':
            ...
            data_name = 'GQA'
            self.images_dir = os.path.join(data_dir, 'GQA')
            ...
            #       len(self.gt_boxes),
            #       len(self.gt_classes),
            #       len(self.relationships)) # 72140 72140 66078 66078 66078
    ...
    def __getitem__(self, index):
        ...
        try:
            image_unpadded = Image.open(os.path.join(self.images_dir, 'images', self.filenames[index])).convert('RGB')
        except:
            print(os.path.join(self.images_dir, self.filenames[index]))
            raise RuntimeError('image file error')
